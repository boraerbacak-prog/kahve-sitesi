...
export default function ComingSoonSection({ products }: { products: ComingSoonProduct[] }) {
  ...
  const days = current.estimatedRoastAt ? daysUntil(current.estimatedRoastAt) : null;
  const isSoon = days !== null && days <= 7;

  return (
    <section
      ref={sectionRef}
      className="relative min-h-screen overflow-hidden bg-[#f7f5f0]/95 border-t border-primary/5"
    >
      ...
      >
        <div className="text-center mb-3">
          <span className="eyebrow">Yakında</span>
          <h2 className="text-lg sm:text-xl lg:text-2xl font-bold text-heading mt-1">
            Yaklaşan <span className="text-primary">Hasatlar</span>
          </h2>
          <p className="text-sm text-body leading-relaxed max-w-xl mx-auto mt-2">
            Kavurma takvimimize girmek üzere olan, limitli seçkin hasatlar. Dinlenme sürecini kargonuzda tamamlayıp en kusursuz kondisyonuyla fincanınıza ulaşacak çekirdekleri önceden keşfedin.
          </p>
        </div>
        ...
        >
          <span className="text-[10px] tracking-[0.25em] uppercase text-primary/80 font-medium">
            {current.origin || "Yeni Hasat"}
          </span>

          <div className="flex justify-center mt-2">
            <span
              className={`text-[11px] font-semibold px-3 py-1 uppercase tracking-wider ${
                isSoon
                  ? "bg-amber-50 text-amber-700 border border-amber-200"
                  : "bg-gray-100 text-gray-500 border border-gray-200"
              }`}
            >
              {days !== null ? `${days} gün sonra` : "Yakında"}
            </span>
          </div>

          <h3 className="text-base sm:text-lg lg:text-xl font-bold text-heading mt-2 leading-tight max-w-md mx-auto">
            {current.name}
          </h3>

          {current.estimatedRoastAt && (
            <p className="text-sm text-muted mt-1">
              Kavrum: {new Date(current.estimatedRoastAt).toLocaleDateString("tr-TR", {
                day: "numeric",
                month: "long",
              })}
            </p>
          ...
          {current.seasonNote && (
            <p className="text-xs text-amber-700 italic mt-1 max-w-xs mx-auto leading-relaxed">
              {current.seasonNote}
            </p>
          )}
        </div>

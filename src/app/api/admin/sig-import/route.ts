...
export async function POST(req: Request) {
  try {
    ...
    const catOptions = ["Aksesuar", "Giyim", "Wood Art"];
    ...
    for (let i = 1; i < lines.length; i++) {
      ...
      const category = row["kategori"] || "Aksesuar";
      const finalCat = catOptions.includes(category) ? category : "Aksesuar";
      const sizesRaw = row["bedenler"] || row["sizes"] || "[]";
      const sizes = sizesRaw.startsWith("[") ? sizesRaw : JSON.stringify(sizesRaw.split(",").map(s => s.trim()).filter(Boolean));

      await prisma.signatureProduct.upsert({
        where: { slug },
        update: {
          ...
          category: finalCat, sizes, sortOrder: parseInt(row["sıra"] || row["sortorder"]) || 0,

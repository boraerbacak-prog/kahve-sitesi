...
export async function GET() {
  const items = await prisma.signatureProduct.findMany({ orderBy: { sortOrder: "asc" } });

  const rows = items.map(p => [
    p.category, p.name, p.slug, tl(p.price), p.salePrice ? tl(p.salePrice) : "",
    p.image,
    (() => { try { return JSON.parse(p.sizes).join(", "); } catch { return p.sizes; } })(),
    String(p.sortOrder), p.published ? "Evet" : "Hayır",
  ]);

  const html = htmlTable("İmza Ürünler", ["Kategori", "Ürün Adı", "Slug", "Fiyat", "İndirimli", "Görsel", "Bedenler", "Sıra", "Yayında"], rows);

  return new NextResponse(html, {
    headers: {
      "Content-Type": "application/vnd.ms-excel; charset=utf-8",
      "Content-Disposition": `attachment; filename="imza-urunler-${new Date().toISOString().slice(0, 10)}.xls"`,
    },
  });
}
